<?php
    ...
    if(empty($_POST['nama']))
    {
         $sql="SELECT DISTINCT penggajian.id_user, USERINFO_DETAIL.nama,penggajian.penempatan,penggajian.bagian FROM penggajian INNER JOIN USERINFO_DETAIL ON penggajian.id_user = USERINFO_DETAIL.ID where penggajian.tanggal between #".$tgl_awal."# and #".$tgl_ahir."#";
    }
    else
    {
        $sql="SELECT DISTINCT penggajian.id_user, USERINFO_DETAIL.nama, penggajian.bagian, penggajian_1.penempatan FROM penggajian AS penggajian_1, penggajian INNER JOIN USERINFO_DETAIL ON penggajian.id_user = USERINFO_DETAIL.ID WHERE (((penggajian.id_user)=".$_POST['nama'].") AND ((penggajian.tanggal) Between #".$tgl_awal."# And #".$tgl_ahir."#))";
    }    
    $query=$db->query($sql);
    $id=array();
    $nama=array();
                    ...
                    <table >
                        ...
                        while ($data=$query1->fetch()) 
                        {
                           ...
                           $simpantampil.="<tr>
                             ...
                             <th '>".$data['Gaji']."</th>  
                             <th '>".$data['total']."</th>
                             <th '>".$data['total1'].$datasementara."</th></tr>";
