...
function _inpComboBox(inp) {
	...
	inp.isi =
		...
		`" ` +
		fdata +
		`  style="width: -moz-available; text-align: left;width: 100%;` +
		inp.attr +
		`">
            ` +
		fdata1 +
...
function _inpDropdonwSelected(inp) {
	...
	inp.data.forEach((v, i) => {
		...
		fdata +=
            ...
            <div class="row" style="margin:5px; text-align:left;">
                <button class="btn ` +
			fbg +
			` btn-sm btn-block" style="text-align:left;" onclick="` +
			fdata1 +
			`">` +
			v.valueName +
			`</button>
	...
	if (inp.inputType != undefined && inp.inputType) {
		return (
			...
			fclassDropdonw +
			`">
                <input type="text" class="btn dropdown-toggle" role="button" id="` +
			fid +
			`" onclick="_showForDropSelect('` +
			inp.id +
			`')" onchange="` +
			inp.inputChange +
			`(this,` +
			...
			inp.id +
			`" style="width: -moz-available; text-align: left;margin: 0px;">
                    <div style="overflow:auto; max-height:200px" id="` +
			inp.idData +
			`">
                        ` +
			fdata +
	...
	return (
		...
		`">
            <p class="btn ` +
		fbg +
		` dropdown-toggle" role="button" id="` +
		fid +
		`" onclick="_showForDropSelect('` +
		inp.id +
		`')" style="width: -moz-available; text-align: left;width: 100%;">
		...
		ffuncSearch +
		`" placeholder="search.." style="background-color:` +
		inp.bgSearch +
		`; color:white;" value="" required="">
                <div style="overflow:auto; max-height:200px" id="` +
		inp.idData +
		`">
                    ` +
...
function _btn(v) {
	...
	return (
		` <button class="` + fclass + `" ` + v.attr + `>` + v.judul + `</button>`
	);
}
function _btnIcon(v) {
	fdata = "";
