function _onload(data) {
	...
	viewWebsite += style_.rowCol({
		...
		col: [
			...
			{
				cls: "-10",
				html: _chartTahapan() + _tabelKecamatan(),
			},
		],
	});
		...
		kolom: [
			{
				...
				form:
					`<div id='tabelShow' class="card">` + // k kemaren
					setTabel() +
					`</div>`,
			},
		],
	...
	modal_.setMo({
		...
		footer:
			...
			_btn({
				...
				judul: "Login",
				attr:
					"style='float:right; padding:5px;;' onclick='_logined(" + key + ")'",
...
function _logined(key) {
	...
	if (_isNull(param.kdDinas))
		return _toast({
			bg: "e",
			msg: "Tentukan OrganisasiPerangkat Daerah Anda !!!",
		});
	if (_isNull(param.username))
		return _toast({ bg: "e", msg: "Tambahkan username !!!" });
	if (_isNull(param.password))
		return _toast({ bg: "e", msg: "Tambahkan password !!!" });
