                ...
                <div class="navbar-header">
                    ...
                    <div class="navbar-brand">
                        <a href="/">
                            <img class="logo" src="` +
		assert +
		`fs_css/BAPPEDA1.png" height="50" alt="logo">
                ...
                <!-- <a class="navbar-brand border-bottom border-top p-2" href="` +
		_.sfUrl +
		`">PARIRI LEMA BARIRI<i></i></a>  -->
						...
						`
                                <div class="col-lg-12"  style="text-align: right;">
                                    <div class="icon topnav">
                                        <a href="` +
						_.sfUrl +
						`" style="color: yellow;" class="bi bi-facebook mr-2" ></a>
                                        <a href="` +
						_.sfUrl +
						`" style="color: yellow;" class="bi bi-instagram mr-2"></a>
                                        <a href="` +
						_.sfUrl +
						`" style="color: yellow;" class="bi bi-mailbox mr-2"></a>
                                    </div>
                                </div>
                                `,
...
function _fiIcon(judul) {
	...
	return (
        ...
        <section id="` +
		fid +
		`" class="content-section" style="width: 100%;">
            <div class="col-` +
		fsize +
		` p-0">
                ` +
					...
					if (v.func != undefined) {
						fdata +=
							...
							_inp({
								...
								attr:
									" onchange='" +
									v.func.substring(0, v.func.length - 1) +
									i1 +
									",this)' " +
									_trueChecked(1, Number(v1["value"])),
							}) +
							`</td>`;
					} else {
	...
	v.data.forEach((v1, i1) => {
		...
		v.kolom.forEach((v2, i2) => {
			actId =
				`id="` +
				(v.id != undefined
					? nmId + (v1.ind == undefined ? v.no - 2 : v1.ind) + "_" + (i2 + 1)
					: "") +
				`"`;
			// f1data=v2.split("$");
			// if(f1data.length==2){
					...
					fdata +=
						...
						_inp({
							...
							attr:
								...
								i1 +
								",this)'",
							checked: _trueChecked(1, Number(v1["checked"])),
						}) +
						`</td>`;
				} else {
					kond = false;
			...
			if (f1data.length == 2) {
				fdata +=
					`<td ${actId}>` +
					_$(Number(v1[f1data[0]]) + Number(v1[f1data[1]])) +
					`</td>`;
				kond = false;
			}
			f1data = v2.split("-");
			...
			if (f1data.length == 2) {
				fdata +=
					`<td ${actId}>` + v1[f1data[0]] + ` ` + v1[f1data[1]] + `</td>`;
				kond = false;
			}
			f1data = v2.split("<tm2>");
			if (f1data.length == 2) {
				fdata +=
					`<td ${actId}>` +
					(v1[f1data[0]].length > 20
						? v1[f1data[0]].substring(0, 20) + "..."
						: v1[f1data[0]]) +
					`</td>`;
				kond = false;
			}
		...
		if (faction) {
			...
			fdata +=
				...
				_btnGroup(v.action, v1.ind == undefined ? v.no - 2 : v1.ind) +
				`</td>`;
		}
		fdata += `</tr>`;
...
function _modalShowImage(v) {
	...
	return _modalCenterContent(
		fjudul,
		`<img src="` + assert + v.file + `" ` + fstyle + `>`
	);
}
...
function _formIcon(v) {
	...
	return (
        ...
        <section id="` +
		fid +
		`" class="content-section bg-light" style="width: 100%;">
            <div class="col-` +
		fsize +
		` p-0">
                ` +
		fdata +
		`
                <div class="card-block" style="padding:10px; background:` +
		fbgForm +
		`;">
                    ` +
		v.isi +
...
function _tabx(v) {
	...
	v.tab.forEach((v1, i1) => {
		...
		if (v1.onclick != undefined) {
			fonclick =
				` onclick="` +
				v1.onclick.substring(1, v1.onclick.length - 1) +
				i1 +
				`)"`;
		}
		ftab +=
...
function _formCollspan(v) {
    ...
    <div class="card shadow mb-4">
        ...
        <a href="#` +
		v.id +
		`" class="d-block card-header py-3 " data-toggle="collapse" role="button" aria-expanded="true" aria-controls="collapseCardExample" ` +
		v.attrHeader +
		`>
            <div class="row">
                <div class="col-auto  mt-auto mb-auto">
...
function _galeryx1(val) {
				...
				kolom: [
					{
						...
						form:
							...
							val.url +
							v.url +
							`" class="stretched-link btn btn-info">Preview page</a>
                                `,
					},
					{
						size: "6",
						form:
							`<img src="` +
							val.assets +
							`assets/fs_sistem/upload/image/` +
							v.img +
							`" class="bd-placeholder-img btn-outline-success my-2 my-sm-0" style="width: inherit;">`,
	...
	return (
                        ...
                        <div class="col-md-4 bg-light p-1">
                            <div class="row no-gutters border rounded overflow-hidden flex-md-row m-1 shadow-sm h-md-250 position-relative">
                                <div class="col-sm-7 p-2 d-flex flex-column position-static">
                                    ...
                                    <div class="mb-1 text-muted">Pengaturan Tahun ` +
		_tahun +
		`</div>
                                    <p class="card-text mb-auto">
                                    ...
                                    <a href="` +
		router +
		`control/member" class="stretched-link">Continue page</a>
                                </div>
                                <div class="col-auto d-none d-lg-block">
                                    <img src="` +
		assert +
		`icon/people.svg" class="bd-placeholder-img btn-outline-success my-2 my-sm-0" width="` +
		width +
		`" height="` +
		height +
		`" >
		...
		`</div>
                                    ...
                                    <a href="` +
		router +
		`control/kunci" class="stretched-link">Continue page</a>
                                </div>
                                <div class="col-auto d-none d-lg-block">
                                    <img src="` +
		assert +
		`icon/key_1.svg" class="bd-placeholder-img btn-outline-success my-2 my-sm-0" width="` +
		width +
		`" height="` +
		height +
		`" >
...
function _galeryx2(val) {
	...
	val.data.forEach((v, i) => {
		...
		fres +=
			...
			"assets/fs_sistem/upload/image/" +
			v.img +
			`"); height: 150px;background-size: cover;width: 100%;'> </div>
                    <div class="btn-secondary" style="width:100%"> 
                        ` +
			_textCenter({
				text:
					`<a onclick="_redirectOpen('` +
					val.url +
					v.file +
					`')" class="stretched-link btn"><h5 class="mb-0">` +
					v.judul +
					`</h5></a>`,
			}) +
			`
