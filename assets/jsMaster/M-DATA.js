function _themaDashboard(v) {
	...
	dropdonw_.dmenu = [];
	dropdonw_.dmenu.push({
		htmlLi: `<a class="dropdown-item" href="${
			router + "control/setting"
		}">pengaturan</a>`,
	});
	dropdonw_.dmenu.push({
		htmlLi: `<a class="dropdown-item" href="${
			router + "control/logout"
		}">logout</a>`,
	});

	header_.hmenu = [];
	header_.hmenu.push({
		htmlLi: `<a href="${router + "control/dashboard/null"}" class="nav-link ${
			v.menu == 1 ? active : noactive
		} text-center">
	...
	header_.hmenu.push({
		htmlLi: `<a href="${
			router + "control/usulan/" + _.tahun
		}" class="nav-link ${v.menu == 2 ? active : noactive} text-center">
            <span class="mdi mdi-collage  d-block mdi-18px"></span>
                <b>usulan</b>
            </a>`,
	});
	header_.hmenu.push({
		htmlLi: `<a href="${router + "control/setting"}" class="nav-link ${
			v.menu == 3 ? active : noactive
		} text-center">
	...
	if (_kdJabatan == 3) {
		header_.hmenu.push({
			htmlLi: `<a href="${_urlMaster + "control/prioritas"}" class="nav-link ${
				v.menu == 3 ? active : noactive
			} text-center" target="_blank">
	...
	return header_.ex3({
		...
		htmlJudul:
          ...
          <a href="/" class="d-flex align-items-center col-md-3 mb-2 mb-md-0 text-dark text-decoration-none">
            <img src="` +
			assert +
			`fs_css/logo/2.png" width="300">
          </a>
        `,
		clsKeterangan:
			"d-flex flex-column  align-items-center my-3 text-white  p-3",
                    ...
                    <div class="col-md-3 align-self-center">
                      ${dropdonw_.ex1({
                              ...
                              <a href="#" class="d-block link-dark text-decoration-none dropdown-toggle judulFunc" data-bs-toggle="dropdown" aria-expanded="false">
                                <img src="${
																	assert + "fs_css/boy.png"
																}" alt="mdo" width="32" height="32" class="rounded-circle">${_nama}
                              </a>`,
											})}
                    </div>
                `,
...
function _themaDashboardNoMenu(v) {
	dropdonw_.dmenu = [];
	dropdonw_.dmenu.push({
		htmlLi: `<a class="dropdown-item" href="#">pengaturan</a>`,
	});
	dropdonw_.dmenu.push({
		htmlLi: `<a class="dropdown-item" href="${
			router + "control/logout"
...
function _flogin() {
	...
	return (
		...
		_inpGroupPrepend({
			...
			attr: ";",
			type: "text",
			icon: '<i class="mdi mdi-home ' + fcolor + '"></i>',
		...
		_inpGroupPrepend({
			...
			attr: ";",
			type: "password",
			icon: '<i class="mdi mdi-key ' + fcolor + '"></i>',
			bg: "bg-info text-light",
		})
	);
}
function _fusulan($bg) {
	...
	return (
		_inpGroupPrepend({
			...
			attr: ";",
			type: "text",
			icon: '<i class="mdi mdi-progress-close ' + fcolor + '"></i>',
			bg: $bg + " text-light",
		}) +
		_inpGroupPrepend({
			id: "uraianPekerjaan",
