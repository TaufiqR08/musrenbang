function _onload(data) {
	...
	$("#footer").html(
		teditor_.lib(assert + "Library/mfc/library/", true) +
			data.footer +
			startmfc.endBootstrapHTML(2)
	);
	setTabel();
	_startTabel("dt");
}
function _formData() {
	return (
		...
		_formIcon({
			...
			btn: !Number(_.key.c)
				? button_.ex2({
						text: `<span class="mdi mdi-form-select "></span> Form Respon`,
						cls: " btn-sm bg-info ",
						attr: ` onclick='_fadd()'`,
				  })
				: "",
	...
	return $("#tabelShow").html(
		_tabelResponsive({
			...
			isi: _tabel({
				data: _.data,
				no: 1,
				kolom: ["nmDinas", "uraian<tm5>"],
				namaKolom: ["Pengirim", "Uraian"],
				action: infoSupport1,
			}),
		})
	);
...
function _fadded() {
	...
	if (_isNull(param.uraian))
		return _toast({ bg: "e", msg: "entri uraian !!!" });
...
function _goDokumen(ind) {
	if (_.data[ind].file.length == 0) {
		return _toast({
			bg: "e",
			msg: "Maaf, respon ini tidak memiliki dokumen !!!",
		});
	}
	_redirectOpen(
		"laporan/previewFile/" + btoa(JSON.stringify({ files: _.data[ind].file }))
	);
}
